...
// 
//   assert Ok(erlexec.ObtainStdout(_, "hi\n")) = erlexec.obtain(1000)
//   assert Ok(erlexec.ObtainStderr(_, "there\n")) = erlexec.obtain(1000)
// 
//   result
//   |> should.be_ok
// 
//   assert Ok(Pids(pid, ospid)) = result
//   stop(ospid)
//   |> should.be_ok
//   stop(9999999)
//   |> should.be_error
// 
//   assert Ok(got_os_pid) = to_ospid(pid)
//   got_os_pid
//   |> should.equal(ospid)
// }
// 
// pub fn stdin_check() {
//   assert Ok(bash) = find_executable("bash")
...
//     |> with_debug(10)
// 
//   assert Ok(Pids(_pid, ospid)) = run_async(options, cmd)
//   assert Ok(erlexec.ObtainStdout(_, "started\r\n")) = erlexec.obtain(5000)
//   assert Ok(_) = erlexec.send(ospid, "test\n")
//   assert Ok(erlexec.ObtainStdout(_, "started\r\n")) = erlexec.obtain(5000)
//   Nil
// }
// 
...
//   assert Ok(erlexec.ObtainStdout(_, "started\r\n")) = erlexec.obtain(5000)
//   assert Ok(_) = erlexec.send(ospid, "test\n")
//   assert Ok(erlexec.ObtainStdout(_, "test\r\n")) = erlexec.obtain(5000)
//   let _ = erlexec.kill_ospid(ospid, 9)
// 
//   // with echo
//   assert Ok(Pids(_pid, ospid2)) =
//     options
//     |> with_pty_echo(True)
//     |> run_async(cmd)
// 
//   assert Ok(erlexec.ObtainStdout(_, "started\r\n")) = erlexec.obtain(1000)
//   assert Ok(_) = erlexec.send(ospid2, "test\n")
//   assert Ok(erlexec.ObtainStdout(_, "test\r\n")) = erlexec.obtain(1000)
//   assert Ok(_) = erlexec.kill_ospid(ospid2, 9)
// }
// 
// pub fn env_check() {
//   assert Ok(env) = find_executable("env")
